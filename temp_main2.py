...
brain1=import_character("mystery/"+character_1+".csv")
brain2=import_character("mystery/"+character_2+".csv")
brain3=import_character("mystery/"+character_3+".csv")
...
def talk_to_ai(id,prompt,director=False):
    brain=brains[id]
    context=brain+memory+[{"role":"user","content":prompt}]
    # context.append({"role":"user","content":prompt})
    response = text_generation(context)
    memory.append({"role":"user","content":"Detective asked : "+prompt})
    memory.append({"role":"assistant","content": character_names[id]+" said : "+response})
    return response 
...
def talk_to_brain(id,who_said,response): # who_said said response
    brain=brains[id]
    character=character_names[who_said]

    context= brain+[{"role":"system","content":character + " said : \""+response+"\". Do you disagree with it? If yes then just repond with \"Agree\". Otherwise Give reason, Why"}]
    out1=text_generation(context)
    out2=None
    if("Agree" not in out1):
        memory.append({"role":"assistant","content": character_names[id] + " said : \"out1\""})
    else:
        out1 = None
        out2=None
    return out1,out2
...
def ai_thread(id, message):
    ...
    for i in range(0,3):
        if id==i: 
            ...
            out1,out2 = talk_to_brain(i,id,response)
            if out1 is None:
                continue
            message_histories[i].config(state=tk.NORMAL)
            message_histories[i].insert(tk.END, f"Suspect: {out1}\n\n")
            message_histories[i].config(state=tk.DISABLED)
            message_histories[i].see(END)
            message_entries[i].delete(0, tk.END)

            if out2 is None:
                continue
            message_histories[id].config(state=tk.NORMAL)
            message_histories[id].insert(tk.END, f"Suspect: {out2}\n\n")
            message_histories[id].config(state=tk.DISABLED)
            message_histories[id].see(END)
            message_entries[id].delete(0, tk.END)
